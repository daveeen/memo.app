...
export default function Songs() {
  ...
  return (
    <div style={cssText("flex:1;min-height:0;display:flex;flex-direction:column;overflow:hidden;padding:24px 22px 88px;animation:mUp .3s ease both;")}>
      <div style={cssText("flex:none;")}>
      ...
      <div className="m-scroll" style={cssText("flex:1;min-height:0;margin-top:20px;display:flex;flex-direction:column;gap:12px;")}>
        {view.map((s) => (
          <button key={s.id} onClick={() => nav(`/songs/${s.id}`)} style={cssText("display:block;width:100%;flex:none;text-align:left;padding:15px;border-radius:16px;border:1px solid rgba(0,0,0,.07);background:#fff;cursor:pointer;box-shadow:0 3px 12px rgba(0,0,0,.04);")}>
