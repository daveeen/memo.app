...
app/
  ...
  lib/
    ...
    audio/
      ...
      audioCache.ts                 # getIdeaAudioBlobUrl/prefetchIdeaAudio: session-lived Map<raw_path, blob
                                    # url> cache for an idea's own recording — cache key is the stable
                                    # raw_path, NOT ideaAudioUrl()'s signed url (that rotates a token every
                                    # call). ideas.$id.tsx and songs.new.tsx both prefetch on mount (2026-07-25)
    opendaw/
...
- **2026-07-25 — Idea audio playback: loading/error states + a session-lived blob-url cache, chasing the "silent on Android PWA" report.** `playback.ts` (Tone.js chord playback) and the two idea-recording playback paths (`ideas.$id.tsx`'s "Play take", `usePreviewPlayer.ts`'s shared preview hook) got matching treatment: a `starting`/loading spinner on the button instead of nothing while `Tone.start()`/sign+fetch/`play()` resolves, an error indicator (⚠) if it fails instead of staying silently dead, and `console.log`/`console.error` at every step (`[playback]`, `[IdeaDetail]`, `[preview]` prefixes) so a remote-debugged Android session actually shows where it's failing — root cause still unconfirmed, no browser available in this build environment.
  - `app/lib/audio/audioCache.ts` (new) caches an idea's own recording by `raw_path` (session-lived `Map`, same pattern as `useCachedFetch.ts`/`arrange.ts`'s `buildCache`) — the actual audio bytes, not just the signed url, since caching by the signed url itself would never hit (`ideaAudioUrl()` mints a fresh token every call). `ideas.$id.tsx` and `songs.new.tsx`'s Chooser strip both prefetch on mount so sign+download latency is hidden before the user taps play; a cache hit plays from a `blob:` url with zero network, which also fixes true "cold play" for a second listen. Deliberately no eviction/LRU (`ponytail:`-marked — recordings are small, session is short); deliberately no service-worker/offline persistence (Cache API was considered — rejected because Workbox caches by full URL by default and the signed-url token would defeat it without extra `cacheKeyWillBeUsed` config, and offline/reload persistence wasn't the ask).
  - `arrange.ts`'s `buildSong()` got the same session-lived cache treatment, keyed by `idea.id:brief.id` — a repeat build for the same pair skips a second Gemini round trip. Deliberate tradeoff: rebuilding the same idea+brief pair for a genuinely different arrangement now needs a page reload to bypass the cache; accepted since repeat-pair builds are almost always an accidental double-click, not an intentional "reroll." `songs.new.tsx`'s `build()` also got a synchronous `buildingRef` guard (same pattern as `playback.ts`'s `activeStop`) against a double-click racing past the `disabled` button before React re-renders it.
